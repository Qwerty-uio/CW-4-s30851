...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(emp => depts.Where(dept => dept.Loc == "CHICAGO").ToList().Find(dept => dept.DeptNo==emp.DeptNo)!=null).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(g => 
            new{
                DeptNo = g.Key,
                Count = g.Count() 
            });
